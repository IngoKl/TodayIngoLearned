...
function tilsObject(tils, user_id) {
    ...
    tils.forEach(element => {
      ...
      tils_combined[key]["date"] = new Date(element.date); // date is in milliseconds
      tils_combined[key]["repetitions"] = element.repetitions;
      tils_combined[key]["last_repetition"] = new Date(element.last_repetition * 1000); // last_repetition is in Unix seconds
